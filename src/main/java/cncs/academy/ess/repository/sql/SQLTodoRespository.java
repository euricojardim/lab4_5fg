...
public class SQLTodoRespository implements TodoRepository {
    private final BasicDataSource dataSource;

    public SQLTodoRespository(BasicDataSource dataSource) {
    ...
    public List<Todo> findAll() {
        try (Connection connection = dataSource.getConnection()) {
            ...
            ResultSet rs = stmt.executeQuery();
            List<Todo> lists = List.of();
            while (rs.next()) {
                lists.add(mapResultSetToTodo(rs));
            }
            return lists;
        } catch (SQLException e) {
    ...
    public List<Todo> findAllByListId(int listId) {
        try (Connection connection = dataSource.getConnection()) {
            ...
            throw new RuntimeException("Failed to find all todos by list ID", e);
        }
    }
